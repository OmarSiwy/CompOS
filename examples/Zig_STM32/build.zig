const std = @import("std");

pub fn build(b: *std.Build) !void {
    const target = b.standardTargetOptions(.{});
    const optimize = b.standardOptimizeOption(.{});
    const target_name: []const u8 = "PIC18F4550";
    const output_path = "tools/pic18f4550_linker.ld";

    const ArtosDep = b.dependency("A_RTOS_M", .{
        .Compile_Target = "STM32F103",
        .Optimization = "ReleaseSafe",
        .Library_Type = "Static",
    });
    const ArtosExe = ArtosDep.artifact("A_RTOS_M");
    const ArtosRun = b.addRunArtifact(ArtosExe);

    // Generate the linker script
    ArtosRun.AddGeneratedLinker(target_name, output_path) catch {
        std.debug.print("Failed to generate linker script for target: {s}\n", .{target_name});
        return;
    };

    // Create an executable with the generated linker script
    const exe = b.addExecutable(.{
        .name = "project.elf",
        .target = target,
        .optimize = optimize,
        .root_source_file = .{ .cwd_relative = "src/main.zig" },
        .linkerscript = output_path,
    });
    exe.linkLibrary(ArtosExe);
    b.installArtifact(exe);

    // Generate Bin File
    const bin = exe.addObjCopy(.{ .format = .bin });
    const installBin = b.addInstallBinFile(bin.getOutput(), "output.bin");
    b.getInstallStep().dependOn(&installBin.step);

    // Generate compile commands for Intellisense
    ArtosRun.AddCompileCommandStep(b, exe);
}
